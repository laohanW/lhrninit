...
if (cli) {
  ...
  case 'init':
    if (args[1]) {
      ...
      var pos, rnSource = 'react-native';
      if ((pos = process.argv.indexOf('--source')) >= 0)
        rnSource = process.argv[pos + 1];

      init(args[1], verbose, {rnSource});
    } else {
      console.error(
        "Usage: react-native init <ProjectName> [--verbose] [--source react-native@0.14.2] or \n" +
        "react-native init <ProjectName> --source git+https://github.com/facebook/react-native.git#v0.14.2"
      );
      process.exit(1);
    }
    break;
...
function init(name, verbose, params) {
  validatePackageName(name);

  if (fs.existsSync(name)) {
    createAfterConfirmation(name, verbose, params);
  } else {
    createProject(name, verbose, params);
  }
}

function createAfterConfirmation(name, verbose, params) {
  prompt.start();
...
function createProject(name, verbose, params) {
  ...
  if (verbose) {
    runVerbose(root, projectName, params);
  } else {
    run(root, projectName, params);
  }
}

function run(root, projectName, params) {
  var pkg = params.rnSource || 'react-native';
  var cmd = 'npm install --save ' + pkg;
